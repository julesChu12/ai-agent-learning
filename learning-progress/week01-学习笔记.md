...
enc = tiktoken.get_encoding("cl100k_base")  # GPT-4o/3.5用的编码
...
### 2.2 主流模型 Context Window（2025年最新）

| 模型 | Context Window | 特点 |
|------|---------------|------|
| **GPT-4o** | 128K | 通用强，性价比高 |
| **GPT-4o-mini** | 128K | 轻量版，成本低 |
| **Claude 3.5 Sonnet** | 200K | 长上下文、编程强 |
| **Claude 3 Opus** | 200K | 最强推理 |
| **Kimi (Moonshot)** | 128K/1M | 中文强、超长上下文 |
| **通义千问 Qwen-Max** | 128K | 中文强 |
...
**选型建议：**
- 开发测试：GPT-4o-mini / Claude 3 Haiku（成本低）
- 生产环境：GPT-4o / Claude 3.5 Sonnet（能力强）
- 长文本处理：Claude 3.5 Sonnet (200K) / Kimi (1M)
...
```python
# 用tiktoken计算Token
# 支持 GPT-4o / GPT-4o-mini / Claude 等模型的成本计算
...
def calculate_cost(text: str, model: str = "gpt-4o-mini") -> float:
    ...
    total_cost = (tokens * input_price + output_tokens * output_price) / 1_000_000
...
for model in ["gpt-4o-mini", "gpt-4o", "claude-3-5-sonnet"]:
    ...
    cost = calculate_cost(text, model)
    print(f"{model}: {tokens} tokens, 预估成本: ${cost:.6f}")
```
...
def chat_loop():
    print("=== LLM Chat CLI (GPT-4o-mini) ===")
    print("输入 'quit' 退出\n")
...
**更强回答：**
> 以GPT-4o-mini为例，定价是$0.15/1M输入Token、$0.60/1M输出Token。我一般用tiktoken库精确计算：
> ```python
...
### Q4: GPT-4o和GPT-4o-mini如何选型？

**回答要点：**
> - GPT-4o-mini：性价比极高，适合大多数场景，成本只有GPT-4o的6%
> - GPT-4o：复杂推理、创意写作等需要更强能力的场景
> - 实际经验：80%的任务用GPT-4o-mini就够了，只有复杂推理才需要GPT-4o
...
| **模型选型不当** | 用最强模型处理简单任务 | 简单任务用GPT-4o-mini，控制成本 |
...
4. 模型选型很重要：简单任务用GPT-4o-mini，复杂任务用GPT-4o
